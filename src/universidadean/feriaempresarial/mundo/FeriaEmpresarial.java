...
public class FeriaEmpresarial {
    ...
    public String metodo1() {
        System.out.println(this.puestos.length);
        //se crean los contadores
        int contadornorte=0;
        int contadorcentro=0;
        int contadoroccidente=0;
        int contadororiente=0;
        int contadorsur=0;

        for(int i=0; i<this.puestos.length;i++){
            if(this.puestos[i].estaOcupado()){
                String nombre=this.puestos[i].darNombreEmpresa();
                for(int j=0; j<this.empresas.size();j++){
                    if(nombre==this.empresas.get(j).darNombre()){
                        if(this.puestos[i].darZona()=="Zona norte"){
                            contadornorte+=this.empresas.get(j).darNumeroPersonasAsistentes();
                        }
                        else if (this.puestos[i].darZona()=="Zona centro"){
                            contadorcentro+=this.empresas.get(j).darNumeroPersonasAsistentes();
                        }
                        else if (this.puestos[i].darZona()=="Zona occidente"){
                            contadoroccidente+=this.empresas.get(j).darNumeroPersonasAsistentes();
                        }
                        else if (this.puestos[i].darZona()=="Zona oriente"){
                            contadororiente+=this.empresas.get(j).darNumeroPersonasAsistentes();
                        }
                        else
                            contadorsur+=this.empresas.get(j).darNumeroPersonasAsistentes();
                    }
                }
        ...
        //se compara cuál zona tiene más expositores, y se retorna esa zona
        if(contadornorte>=contadorcentro && contadornorte>=contadoroccidente
        && contadornorte>=contadororiente && contadornorte>=contadorsur){
            return "Zona norte tiene "+contadornorte+" expositores";
        }

        else if(contadorcentro>=contadornorte && contadorcentro>=contadoroccidente
                && contadorcentro>=contadororiente && contadorcentro>=contadorsur){
            return "Zona centro";
        }

        else if(contadoroccidente>=contadornorte && contadoroccidente>=contadorcentro
                && contadoroccidente>=contadororiente && contadoroccidente>=contadorsur){
            return "Zona occidente";
        }

        else if(contadororiente>=contadornorte && contadororiente>=contadorcentro
                && contadororiente>=contadoroccidente && contadororiente>=contadorsur){
            return "Zona oriente";
        }
        else
        return "Zona sur";
...
// se uso el método porcentaje ocupación y se comparo con 60, para mirar el punto de equilibrio.


    public String metodo2() {
        if(porcentajeOcupacion()>=60){
            return "Se ha alcanzado el punto de equilibrio";
        }
        else
            return "No se ha alcanzado el punto de equilibrio";
